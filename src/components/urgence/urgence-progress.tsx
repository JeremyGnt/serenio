            ...
            return (
              <div key={step.id} className="flex-1 flex flex-col items-center">
                ...
                <div className="flex items-center w-full">
                  {/* Ligne gauche (ou espace invisible pour le premier) */}
                  {isFirst ? (
                    <div className="flex-1" />
                  ) : (
                    <div
                      className={`flex-1 h-0.5 transition-all duration-700 ease-in-out ${index <= currentStep ? 'bg-red-500' : 'bg-gray-200'
                        }`}
                    />
                  )}
                  ...
                  {/* Ligne droite (ou espace invisible pour le dernier) */}
                  {isLast ? (
                    <div className="flex-1" />
                  ) : (
                    <div
                ...
                <div className="w-full flex justify-center mt-2">
                  <span
                    className={`text-xs text-center leading-tight transition-all duration-700 ease-in-out ${isCompleted
